...
의존성 배열을 잘 관리하지 못해서 발생한 버그는 디버깅이 힘들다. 따라서 제대로 이해하고 사용해야한다.

<br/>

### 의존성 배열 관리

`useEffect` 훅에서 의존성 배열의 내용이 변경됐을 때 부수 효과 함수가 실행된다. 

의존성 배열은 잘못 관리하면 쉽게 버그로 이어지므로 입력하지 않는게 좋다. 다행히 의존성 배열은 대부분의 경우에 입력하지 않아도 되는데, 꼭 필요한 경우가 생기기도 한다.

몇 가지 예제를 통해 의존성 배열을 관리하는 방법을 알아보자.

<br/>

#### 1. 부수 효과 함수에서 API를 호출하는 경우
